...
#define PACKET_SIZE 85*3+6

static usbd_device *usbd_dev;
uint8_t frame_buf[PACKET_SIZE];
uint8_t * ptr_frame_buf = frame_buf;
...
static void cdcacm_data_rx_cb(usbd_device *usbd_dev, uint8_t ep)
{
	...
	int len = usbd_ep_read_packet(usbd_dev, 0x01, ptr_frame_buf, 64);
    ptr_frame_buf += len;

	if ((ptr_frame_buf-frame_buf)>=PACKET_SIZE) {
        ptr_frame_buf = frame_buf;
        usb_rx_callback(frame_buf, PACKET_SIZE);
	}
}
